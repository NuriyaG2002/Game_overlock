...
#класс-родитель для других спрайтов - суперкласс
...
class Player(GameSprite):
    ...
    #метод "выстрел" (вызывается при нажатии на пробел)
    def fire(self):
...
class Bullet(GameSprite):
    def update(self):
        self.rect.y -= self.speed
        #исчезает, если дойдёт до верхнего края экрана
        if self.rect.y < 0:
            self.kill()
...
finish = False #переменная "игра закончилась"
...
while run:
    #событие нажатия на кнопку “Закрыть окно”
    for e in event.get():
        if e.type == QUIT:
            run = False
